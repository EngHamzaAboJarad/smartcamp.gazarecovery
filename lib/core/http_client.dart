...
/// Usage:
///   final res = await HttpClient.instance.get('otp/login', queryParameters: {...});
class HttpClient {
  HttpClient._();

  // Simple in-memory otp token. You can persist this in secure storage and restore on app start.
  ...
  static void _configureInterceptors() {
    // Insert otp injector first so it's applied before logging.
  ...
  /// Set otp token used by the Authorization header for requests.
